...
class wispgg extends HostingModule {
    ...
    private function buildEggVariablesFromEgg($egg) {
        ...
        // Try to find variables in different possible locations
        if (isset($egg['relationships']['variables']['data']) && is_array($egg['relationships']['variables']['data'])) {
            // Format 1: From getEggWithIncludes() - relationships->variables->data
            $variablesData = $egg['relationships']['variables']['data'];
            $this->debugLog("[AUTO-FETCH] Found variables in relationships.variables.data");
        } elseif (isset($egg['data']) && is_array($egg['data'])) {
            // Format 2: Direct API call to /variables endpoint - data[]
            $variablesData = $egg['data'];
            $this->debugLog("[AUTO-FETCH] Found variables in data[]");
        }
        ...
        foreach ($variablesData as $var) {
            ...
            $this->debugLog("[AUTO-FETCH] Found variable: {$envVar} = " . (empty($defaultValue) ? '(empty)' : $defaultValue));
...
public function getNodeAndAllocations() {
    ...
    public function getEggWithIncludes($nest_id, $egg_id) {
        ...
        // Method 1: Try to include variables in the egg call
        ...
        $eggData = $egg['attributes'];
        $this->debugLog("[AUTO-FETCH] Successfully fetched egg: " . $eggData['name']);
        
        // Method 2: If variables not included, fetch them separately
        if (!isset($eggData['relationships']['variables']['data']) || empty($eggData['relationships']['variables']['data'])) {
            $this->debugLog("[AUTO-FETCH] Variables not in include, fetching separately...");
            
            $variables = $this->api('nests/' . $nest_id . '/eggs/' . $egg_id . '/variables');
            
            if ($variables && isset($variables['data'])) {
                $this->debugLog("[AUTO-FETCH] Successfully fetched " . count($variables['data']) . " variables separately");
                // Add variables to egg data in the same format as include would
                $eggData['relationships']['variables']['data'] = $variables['data'];
            } else {
                $this->debugLog("[AUTO-FETCH] WARNING: Could not fetch variables separately");
            }
        } else {
            $varCount = count($eggData['relationships']['variables']['data']);
            $this->debugLog("[AUTO-FETCH] Egg includes {$varCount} variables");
        }
